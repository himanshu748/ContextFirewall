...
export function MemoryCard({
  ...
  return (
    ...
    >
      ...
      <p className={`text-sm leading-snug ${blocked ? "text-slate-300" : "text-slate-200"}`}>
        {v.text}
      </p>
      ...
      {blocked && onForget && (
        <button
          ...
          disabled={forgetting}
          className="mt-3 inline-flex items-center gap-1.5 rounded-md border border-ink-700 bg-ink-800 px-2.5 py-1 text-[11px] font-medium text-slate-400 transition-colors hover:border-block-border hover:text-block disabled:opacity-50"
